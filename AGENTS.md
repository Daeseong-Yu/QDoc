...
- Use `docs/release-go-no-go.md` for operator-facing release evidence procedure changes; do not bury staging, backup, rollback, or GO/NO-GO evidence rules only in chat or ignored `.ai` state.
...
- Treat the current Step 5 backlog as the complete pre-GO task board for the documented portfolio demo, not merely a next-step plan. Staff access, provider-backed map interaction, first-time OTP delivery, public patient/staff smoke, deployed notification/outbox behavior, backup restore-check, rollback target, and safe GO/NO-GO evidence must all be closed or explicitly excluded by the user before `GO`.
...
- Before `GO`, OTP delivery and user-facing errors must be understandable to a non-technical visitor. Do not expose raw internal error codes such as `rate_limited`, `otp_delivery_unavailable`, `invalid_otp`, `expired_otp`, `map_provider_disabled`, `map_budget_exhausted`, `queue_closed`, `invalid_request`, `invalid_transition`, `internal_error`, `not_found`, raw JSON auth tokens such as `"unauthorized"` or `"forbidden"`, or map cost-guardrail implementation details directly in the UI.
...
- For an existing staging database, prefer the staff-admin bootstrap path (`deploy/bootstrap-staff-admins.sh` or `pnpm db:bootstrap-staff-admins:staging`) over rerunning the full seed when the goal is only to add or promote real staff/admin emails. Run dry-run first; the deploy helper defaults to dry-run unless `QDOC_BOOTSTRAP_STAFF_CONFIRM=apply` is set, and production-like apply requires that confirmation. This path must not reset tickets, queues, map settings, or provider guardrail rows, and it must not print real email addresses. Use `pnpm verify:staff-bootstrap` for the DB-free bootstrap input-policy self-test.
...
- P5-A is not complete until a real OTP-receivable staff/admin and tester path is proven without direct database edits. P5-B is not complete while the public map is seeded-clinic-only or non-interactive, and local provider-map regressions must be fixed before collecting staging/public provider evidence. P5-C is not complete while first-time staging OTP delivery or readable auth failures are unproven. P5-D is not complete while patient/staff smoke requires developer explanation. P5-E is not complete while staging verifier, rehearsal, backup restore-check, rollback target, or decision evidence are missing.
...
- After a documentation-fill request, treat those same files as the synchronized work board. If they already contain the durable P5 scope, add only the smallest ownership/status clarification needed; do not duplicate the same evidence or candidate state across `.ai/core`, `AGENTS.md`, and operator docs.
...
- Use `pnpm portfolio:evidence-commands` to print the ordered SSM deploy-document sync, one-time SSM document create fallback, public release preflight, public browser smoke, staging verifier, staff bootstrap, staff/admin expectation verifier, rehearsal, smoke rollup, and release evidence command plan for the current candidate before public/staging evidence collection. It fills `git rev-parse HEAD` when available, carries `QDOC_ADMIN_DATA_EXPECT_SITE_IDS` into staging admin-data verification and staff bootstrap examples, renders `QDOC_SSM_DOCUMENT_NAME`, and warns if the local branch is ahead of upstream; it does not replace syncing or first-creating the default SSM document, pushing, deploying, or checking `/api/release`. After changing that helper, run `pnpm portfolio:evidence-commands:self-test`.
...
- Before running release preflight, portfolio smoke, rehearsal, or evidence commands that require a candidate SHA, read the current value with `git rev-parse HEAD`; do not reuse an older SHA from chat history, release notes, or stale `.ai` status. If the local HEAD changed, update `.ai/execution/current.md` and the active Step 5 status before collecting evidence.
- Use `pnpm e2e:portfolio` for read-only public-browser smoke against a deployed URL. It requires `QDOC_PUBLIC_URL` or `QDOC_PORTFOLIO_BASE_URL`; add `QDOC_PORTFOLIO_EXPECT_PROVIDER_MAP=true` when P5-B must prove a live provider-backed map instead of the fallback map. In strict provider mode, the smoke must prove browser geolocation, provider map ready state, at least one QDoc clinic marker, at least one provider nearby discovery place, a direct drag gesture on the ready provider map surface, and app-level provider pan/zoom/recenter responses. In fallback mode, it must still prove fallback pan/zoom/recenter behavior rather than accepting a static map.
...
- Before collecting public P5 evidence, confirm the current candidate is pushed, deployed, and served by the public URL. `/api/release` must match `git rev-parse HEAD`; if it does not, treat public smoke/release evidence as blocked by stale deployment rather than as product proof.
...
- When the user asks to fill newly defined work into project documents, update the smallest matching document set by ownership: `.ai/core` for durable product/architecture/ADR decisions, `.ai/phases` and `.ai/execution/current.md` for active workflow state, `docs/portfolio-go-work-packages.md` plus `.ai/docs/portfolio-go-work-packages.md` for package handoff, `docs/release-go-no-go.md` or README for operator/tester procedures, and `AGENTS.md` only for rules that must guide future sessions.
- After filling those documents, keep the work in launch-candidate Step 5 unless the user explicitly approves a phase change. Do not duplicate volatile candidate SHA values, real email addresses, provider identifiers, private host paths, or one-off staging logs into durable docs.
- Current documentation-fill checkpoint: the P5-A through P5-E board is already recorded across the active `.ai` state, package handoff, release checklist, README, and this file. Future sessions should update only the owning document for the kind of change being made, then mirror package-level changes between `docs/portfolio-go-work-packages.md` and `.ai/docs/portfolio-go-work-packages.md`.
- For "what remains before GO" or newly defined portfolio-completion work, use the Pre-GO Functional Completion List in `.ai/execution/current.md`, `.ai/phases/qdoc-launch-candidate/step5.md`, and `docs/portfolio-go-work-packages.md`. Treat that list as the complete product-completion backlog for documented demo flows, not as a deployment-only checklist or a plan for only the next small step.
