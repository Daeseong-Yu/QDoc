...
- Use `pnpm portfolio:evidence-commands` to print the ordered SSM deploy-document sync, public release preflight, public browser smoke, staging verifier, staff bootstrap, staff/admin expectation verifier, rehearsal, smoke rollup, and release evidence command plan for the current candidate before public/staging evidence collection. It fills `git rev-parse HEAD` when available, carries `QDOC_ADMIN_DATA_EXPECT_SITE_IDS` into staging admin-data verification and staff bootstrap examples, renders `QDOC_SSM_DOCUMENT_NAME`, and warns if the local branch is ahead of upstream; it does not replace syncing the default SSM document, pushing, deploying, or checking `/api/release`. After changing that helper, run `pnpm portfolio:evidence-commands:self-test`.
