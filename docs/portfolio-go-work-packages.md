...
Latest local verification note: local P5-B recovery and direct provider-map drag gesture proof are committed through `670c25f` (`test: prove provider map drag gesture`). Targeted provider-map E2E, public smoke listing, `pnpm typecheck`, `pnpm lint`, workflow validation/check, and `git diff --check` passed for the drag-gesture update. Earlier targeted provider-map E2E passed 4 tests and full local Playwright E2E passed 34 tests after fixing Mapbox/Google SDK readiness retry and overlapping marker hit testing/selection layering. Staging/public-browser provider evidence is still required with real provider credentials/restrictions, QDoc cost limits, browser geolocation, provider discovery, pan/zoom/recenter/selection behavior, and fail-closed states.
...
| P5-B Provider map public-browser verification | Provider and fallback map interaction paths are implemented and locally green for provider SDK retry, app-level pan/zoom/recenter, marker/card sync, active QDoc site layering, selected-state feedback, direct provider drag-gesture proof, hidden guardrail copy, and disabled-provider fallback coverage. | Verify real provider credentials/restrictions, QDoc cost limits, browser geolocation, provider discovery, public-browser pan/drag behavior, and fail-closed states. | No |
