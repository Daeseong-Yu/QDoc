...
Latest local verification note: local P5-B recovery and direct provider-map drag gesture proof are committed through `670c25f` (`test: prove provider map drag gesture`). Follow-up provider-map SDK integration is covered by focused provider-map E2E, Google-provider guardrail E2E, full local Playwright E2E, `pnpm build`, `pnpm typecheck`, `pnpm lint`, and `git diff --check`. Coverage includes rejected Mapbox and Google reservations without SDK requests, successful Mapbox reservation response before CSS/JS SDK requests, Google SDK reservation/load/control wiring, Google SDK QDoc/provider marker selection and direct-drag proof, Google API config/usage guardrails, provider map interaction, and first-failed-SDK-load recovery. The evidence command helper warns when the worktree has uncommitted changes, and `pnpm portfolio:evidence-commands:self-test` covers that dirty-candidate guard, so public evidence collection must use a committed and deployed candidate SHA. Staging/public-browser provider evidence is still required with real provider credentials/restrictions, QDoc cost limits, browser geolocation, provider discovery, pan/zoom/recenter/selection behavior, and fail-closed states.

Latest staging verification note: the pushed and deployed candidate passed the exact-SHA public release preflight and all four desktop/mobile fallback public-smoke checks. Strict provider smoke failed closed because the staging provider is disabled. This confirms the fallback path but does not close P5-B or P5-D; approved provider credentials/restrictions and budgets, authenticated smoke, real inboxes, and deployed worker evidence remain required. Candidate-specific identifiers and command output stay in active `.ai` state rather than this handoff.

Latest approved P5-B staging configuration: use Mapbox for the public portfolio environment with QDoc monthly hard stops of `5000` `map_load` reservations and `1000` `places_search` reservations. Keep `hardStopEnabled=true`, retain the existing request rate limits and 600-second search cache, restrict the distinct browser token to the public origin, and keep the distinct search token server-side. The shared staging env must select and enable Mapbox, but the existing database guardrail row remains authoritative and must also be enabled through the authorized staff map-settings flow; do not use a hidden direct database edit. No credential value belongs in this handoff or its evidence.
...
| --- | --- | --- | --- |
| P5-A Staff demo access | Bootstrap paths, DB-free staff bootstrap policy and deploy-wrapper self-tests, staff/admin expectation checks, membership management, role-boundary E2E, audit-log E2E, readable unrostered staff guidance, and patient check-in fallback navigation exist locally. | Real OTP-receivable staff/admin inbox, tester membership, unrostered denial, and staging admin-data proof. | No |
| P5-B Provider map public-browser verification | Provider and fallback map interaction paths are implemented and locally green for provider SDK retry, Mapbox rejected and successful map-load reservation before SDK CSS/JS requests, Google SDK reservation/load/control wiring, Google-provider API config/usage guardrail coverage, Google SDK marker/card sync and direct-drag proof, app-level pan/zoom/recenter, marker/card sync, active QDoc site layering, selected-state feedback, direct provider drag-gesture proof, hidden guardrail copy, and disabled-provider fallback coverage. The deployed fallback smoke passes on desktop/mobile. Mapbox and QDoc limits of `5000` map loads and `1000` place searches are approved for staging. | Configure restricted browser and server-side search credentials outside Git, apply the approved env values, enable the database guardrail row through authorized staff map settings with hard stop on, then verify browser geolocation, provider discovery, public-browser pan/drag behavior, cost controls, and fail-closed states. | No |
...
| P5-D Public demo smoke | Local Playwright covers patient/staff core paths, queue operations, membership, audit logs, notification preference, outbox creation/failure visibility, duplicate prevention, and a tester-facing smoke guide now exists. The read-only deployed fallback smoke passes on desktop/mobile without requesting OTP. | Strict provider public smoke, full visitor-style patient smoke, authorized staff smoke, real inbox/session proof, and deployed worker/outbox proof. | No |
...
- `pnpm verify:staff-bootstrap` runs DB-free staff bootstrap checks for required email input, invalid email rejection, staging/production placeholder rejection, production-like apply confirmation, dry-run behavior, site-ID fallback/override parsing, bootstrap plan counts, deployed Compose-wrapper env-file fallback behavior, and safe output that omits real email addresses.
...
- The staging portfolio provider decision is Mapbox. Approved QDoc monthly hard stops are `5000` `map_load` reservations and `1000` `places_search` reservations, with the existing request rate limits, 600-second search cache, and hard stop retained.
...
- Local E2E uses a stubbed provider SDK/cache path, including a first-load script failure retry case, app-level provider pan proof, and direct provider-map drag gesture proof, to avoid paid provider traffic.
...
- Latest P5-B verification update: local provider-map recovery is green again. Targeted provider-map E2E passed 4 tests and the full local Playwright suite passed 34 tests after fixing provider SDK readiness retry and marker hit testing/selection layering. Follow-up default focused provider-map SDK E2E passed 12 tests with the Google-provider API guard skipped under the Mapbox runtime, and the `MAP_PROVIDER=google` guardrail run passed 2 tests for Google config/usage reservation and budget exhaustion. The Mapbox path now proves a successful map-load reservation response occurs before CSS/JS SDK requests, and the Google SDK path now also proves provider marker selection, QDoc marker/card synchronization, app-level direct drag capture, pan, zoom, current-location recentering, and no Google SDK request when map-load reservation is rejected. A subsequent full local `pnpm e2e` after the Mapbox success-path assertion passed 40 tests with 2 expected Google-provider guard skips under the default Mapbox runtime. Return to staging/public proof next; local success does not replace real provider credential, restriction, geolocation, pan/drag, and cost-control evidence.
...
- Public browser Playwright smoke is available through `pnpm e2e:portfolio`. It is read-only, requires `QDOC_PUBLIC_URL` or `QDOC_PORTFOLIO_BASE_URL`, and can fail early on stale public deployments with `QDOC_PORTFOLIO_EXPECT_RELEASE_SHA=<40-character-git-sha>` by comparing the safe `/api/release` identity endpoint. It can require provider-backed map behavior with `QDOC_PORTFOLIO_EXPECT_PROVIDER_MAP=true`. Strict provider mode now fails unless browser geolocation is available, the provider map reaches ready state, at least one QDoc clinic marker is present, at least one provider nearby discovery place is rendered, a direct drag gesture reaches the ready provider map surface, and the app-level pan, zoom, and current-location recenter controls call the live provider map. The smoke also asserts patient OTP-entry surface visibility, clinic-card/marker selected-state feedback, QDoc marker-to-card synchronization, provider marker selection, provider-selection clearing after clinic reselection, and fallback pan/zoom/recenter viewport changes so a map that renders but does not respond to interaction remains a blocker.

Remaining evidence:

- Configure the distinct Mapbox browser token with public-origin restrictions and keep the distinct search token server-side; never copy either value into repository evidence.
- Apply the approved `5000` `map_load` and `1000` `places_search` monthly hard stops in both the shared staging environment and the authorized database-backed staff map settings, with `hardStopEnabled=true`.
...
- `deploy/portfolio-evidence-commands.sh` and `pnpm portfolio:evidence-commands` print the ordered SSM deploy-document sync, one-time SSM document create fallback, public release preflight, public browser smoke, staging verifier, staff bootstrap, staff/admin expectation verifier, rehearsal, smoke rollup, and release evidence commands for the current candidate without contacting AWS, Docker, SMTP, map providers, the public URL, or the database. The helper fills the current Git SHA when available, carries the configured/default launch site IDs into staging admin-data verification and staff bootstrap examples, renders the configured/default SSM document name, separates local repository-checkout commands from deployed-host `/opt/qdoc/current` commands, and warns when the local branch is still ahead of its upstream or the worktree has uncommitted changes, reducing missing/stale SSM-document, wrong-directory, placeholder-SHA, image-tag, stale-clinic-data, and P5-A staff/admin expectation mistakes before evidence collection.
- `deploy/portfolio-evidence-commands.self-test.sh` and `pnpm portfolio:evidence-commands:self-test` cover the command plan locally without AWS, Docker, SMTP, map provider, public URL, or database access. The self-test proves SSM document sync and one-time create command rendering, repository-root command hints, candidate SHA/image rendering, dirty-worktree warning, default and custom launch site IDs, staff email placeholder safety, and provider-map expectation propagation.
