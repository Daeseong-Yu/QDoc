...
- Latest P5-B verification update: the provider SDK failed-load recovery case now passes after the app retries SDK script loads with a cache-busting request, the E2E Mapbox SDK stub prevents stale browser-cache reuse, and the app avoids consuming Mapbox map-load quota before the SDK is available. Full local Playwright now passes; keep P5-B open for staging/public proof.
