...
Before rerunning staging deploy after a change under `deploy/` or `compose.staging.yaml`, run local `pnpm` evidence/deploy helpers from the QDoc repository checkout, not from `/opt/qdoc/current` or an arbitrary shell directory. Synchronize the default SSM deploy document with `AWS_REGION=<region> pnpm deploy:sync-ssm-document`. If the SSM document does not exist yet, create it once with `QDOC_SSM_CREATE_DOCUMENT=true AWS_REGION=<region> pnpm deploy:sync-ssm-document`. The normal sync/update path needs `ssm:DescribeDocument`, `ssm:GetDocument`, `ssm:UpdateDocument`, and `ssm:UpdateDocumentDefaultVersion`; the one-time create path also needs `ssm:CreateDocument`. A stale or missing default SSM document can reject a newer ops bundle before the EC2 command reaches the app deploy script.
...
The helper fills the candidate SHA from `git rev-parse HEAD` when available, prints both the normal SSM deploy-document sync prerequisite and the one-time SSM document create fallback, and warns if the local branch is still ahead of its upstream or the worktree has uncommitted changes. Use it to avoid copying the literal `<git-sha>` placeholder into rehearsal, public release preflight, or release evidence commands, and prefer the printed `QDOC_SSM_CREATE_DOCUMENT=true` helper command over hand-written relative `file://deploy/ssm/...` AWS CLI commands when the document is missing.
